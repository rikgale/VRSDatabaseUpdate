...
UPDATE Aircraft
SET UserBool1 = 1
WHERE RegisteredOwners LIKE '%Air Force%'
   OR RegisteredOwners LIKE '%Navy%'
   OR RegisteredOwners LIKE '%Army%'
   OR RegisteredOwners LIKE '%Rosgvardiya%'
   OR RegisteredOwners LIKE '%Military%'
   OR RegisteredOwners LIKE '%Coast Guard%';
...
-- Reset Last Mod time where OpFlag contains listed, or (is 3 chars long and not military)
-- and only for specific aircraft types
UPDATE "Aircraft"
SET "LastModified" = "FirstCreated"
WHERE ("OperatorFlagCode" IN ('BLANK', 'BLANK1', 'BLANK2')
       OR (LENGTH("OperatorFlagCode") = 3 AND "UserBool1" <> 1))
